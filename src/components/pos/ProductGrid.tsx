...
export default function ProductGrid({ onAddToCart, categories, refreshKey }: ProductGridProps) {
  ...
  return (
    <div className="flex flex-col h-full space-y-4">
      ...
      <div className={`relative transition-all duration-200 ${scanFlash ? 'ring-2 ring-green-500 rounded-2xl' : ''}`}>
        <input
          ref={searchRef}
          type="text"
          placeholder="Scan barcode or search products..."
          className="w-full p-4 pl-12 bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700 outline-none text-lg transition-shadow focus:ring-2 focus:ring-indigo-500 focus:border-transparent"
          value={search}
                ...
                <button
                  ...
                  className={`group flex flex-col items-center justify-center p-5 rounded-2xl shadow-sm transition-all border border-transparent relative ${
                    outOfStock
                      ...
                      : 'bg-white dark:bg-gray-800 hover:shadow-xl active:scale-95 hover:border-indigo-500/50'
                  }`}
                >
                  {/* Low stock badge */}
                  {lowStock && (
