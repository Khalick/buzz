        ...
        <>
          ...
          <div className="p-4 border-b border-gray-200 dark:border-gray-800 flex items-center justify-between">
            <h2 className="text-lg font-bold dark:text-gray-100 flex items-center gap-2">
              <Package size={18} className="text-indigo-500" /> Products
            ...
            <button onClick={openNew} className="px-3 py-1.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg text-xs font-bold transition-colors flex items-center gap-1">
              <Plus size={14} /> New
            </button>
          </div>
          ...
          <div className="px-4 py-2">
            <input
              ...
              className="w-full px-3 py-2 bg-gray-50 dark:bg-gray-800 rounded-lg text-sm outline-none border border-gray-200 dark:border-gray-700 focus:ring-2 focus:ring-indigo-500"
            />
          </div>
            ...
            {filtered.length === 0 ? (
              ...
              filtered.map(p => (
                <div key={p.id} onClick={() => openEdit(p)}
                  className={`flex items-center gap-3 px-3 py-3 rounded-xl cursor-pointer transition-all mb-1 border border-transparent hover:border-indigo-500/30 ${
        ...
        <>
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-4">
            ...
            <div>
              ...
              <input value={form.name} onChange={e => setForm({ ...form, name: e.target.value })}
                placeholder="e.g. Nyama Choma Special" className="w-full px-4 py-3 bg-gray-50 dark:bg-gray-800 rounded-xl text-sm outline-none border border-gray-200 dark:border-gray-700 focus:ring-2 focus:ring-indigo-500" />
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <input type="number" value={form.price} onChange={e => setForm({ ...form, price: e.target.value })}
                  placeholder="0.00" className="w-full px-4 py-3 bg-gray-50 dark:bg-gray-800 rounded-xl text-sm outline-none border border-gray-200 dark:border-gray-700 focus:ring-2 focus:ring-indigo-500" />
              </div>
              <div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <input type="number" value={form.stock_quantity} onChange={e => setForm({ ...form, stock_quantity: e.target.value })}
                  placeholder="0" className="w-full px-4 py-3 bg-gray-50 dark:bg-gray-800 rounded-xl text-sm outline-none border border-gray-200 dark:border-gray-700 focus:ring-2 focus:ring-indigo-500" />
              </div>
              <div>
                ...
                <select value={form.category_id} onChange={e => setForm({ ...form, category_id: e.target.value })}
                  className="w-full px-4 py-3 bg-gray-50 dark:bg-gray-800 rounded-xl text-sm outline-none border border-gray-200 dark:border-gray-700 focus:ring-2 focus:ring-indigo-500 appearance-none">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <input value={form.barcode} onChange={e => setForm({ ...form, barcode: e.target.value })}
                  placeholder="Scan or type..." className="w-full px-4 py-3 bg-gray-50 dark:bg-gray-800 rounded-xl text-sm outline-none border border-gray-200 dark:border-gray-700 focus:ring-2 focus:ring-indigo-500" />
              </div>
              <div>
                <label className="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-1">SKU</label>
                <input value={form.sku} onChange={e => setForm({ ...form, sku: e.target.value })}
                  placeholder="e.g. NC-001" className="w-full px-4 py-3 bg-gray-50 dark:bg-gray-800 rounded-xl text-sm outline-none border border-gray-200 dark:border-gray-700 focus:ring-2 focus:ring-indigo-500" />
              </div>
            </div>
            ...
            <div className="flex items-center justify-between px-4 py-3 bg-gray-50 dark:bg-gray-800 rounded-xl">
              ...
              <button onClick={() => setForm({ ...form, is_active: !form.is_active })}
                className={`w-12 h-6 rounded-full transition-colors relative ${form.is_active ? 'bg-green-500' : 'bg-gray-300 dark:bg-gray-600'}`}>
          ...
          <div className="p-4 border-t border-gray-200 dark:border-gray-800">
            <button onClick={handleSubmit} disabled={!form.name || !form.price}
              className="w-full py-3 bg-gradient-to-r from-indigo-600 to-blue-600 hover:from-indigo-700 hover:to-blue-700 text-white font-bold rounded-xl shadow-lg disabled:opacity-50 transition-all flex items-center justify-center gap-2">
