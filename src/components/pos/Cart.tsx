  ...
  return (
    <div className="flex flex-col h-full bg-white dark:bg-gray-900 border-l border-gray-200 dark:border-gray-800 shadow-xl">
      ...
      <div className="p-4 border-b border-gray-200 dark:border-gray-800 flex justify-between items-center">
        <h2 className="text-lg font-bold bg-gradient-to-r from-[#0D1F16] to-[#1B4332] bg-clip-text text-transparent drop-shadow-sm">
          Current Order
        </h2>
      ...
      <div className="flex gap-1.5 px-4 py-2 border-b border-gray-100 dark:border-gray-800">
        <button onClick={() => setShowDiscount(!showDiscount)}
          className={`px-2.5 py-1 rounded-lg text-[11px] font-bold transition-colors flex items-center gap-1 ${
            discount.type !== 'none' ? 'bg-[#2D6A4F]/10 dark:bg-[#2D6A4F]/30 text-[#2D6A4F] dark:text-[#52B788]' : 'bg-gray-100 dark:bg-gray-800 text-gray-500 hover:bg-gray-200'
          }`}>
          <Percent size={10} /> {discount.type !== 'none' ? 'Discount ✓' : 'Discount'}
        </button>
        <button onClick={() => setShowCustomer(!showCustomer)}
          className={`px-2.5 py-1 rounded-lg text-[11px] font-bold transition-colors flex items-center gap-1 ${
            customerName ? 'bg-[#1B4332]/10 dark:bg-[#1B4332]/30 text-[#2D6A4F] dark:text-[#52B788]' : 'bg-gray-100 dark:bg-gray-800 text-gray-500 hover:bg-gray-200'
          }`}>
          <User size={10} /> {customerName || 'Customer'}
        </button>
        <button onClick={() => setShowNotes(!showNotes)}
          className={`px-2.5 py-1 rounded-lg text-[11px] font-bold transition-colors flex items-center gap-1 ${
            orderNotes ? 'bg-[#D4AF37]/10 dark:bg-[#D4AF37]/20 text-[#D4AF37] dark:text-[#D4AF37]' : 'bg-gray-100 dark:bg-gray-800 text-gray-500 hover:bg-gray-200'
          }`}>
      ...
      {/* Discount panel */}
      {showDiscount && (
        <div className="px-4 py-3 border-b border-gray-100 dark:border-gray-800 bg-[#2D6A4F]/5 dark:bg-[#2D6A4F]/10 animate-fade-in space-y-2">
          <div className="flex gap-2">
            ...
            <button onClick={() => applyDiscount('percent')} className={`px-3 py-2 rounded-lg text-xs font-bold transition-colors ${discount.type === 'percent' ? 'bg-[#2D6A4F] text-white' : 'bg-gray-100 dark:bg-gray-800 text-gray-600'}`}>%</button>
            <button onClick={() => applyDiscount('fixed')} className={`px-3 py-2 rounded-lg text-xs font-bold transition-colors ${discount.type === 'fixed' ? 'bg-[#2D6A4F] text-white' : 'bg-gray-100 dark:bg-gray-800 text-gray-600'}`}>KES</button>
            {discount.type !== 'none' && (
      ...
      {/* Customer panel */}
      {showCustomer && (
        <div className="px-4 py-3 border-b border-gray-100 dark:border-gray-800 bg-[#1B4332]/5 dark:bg-[#1B4332]/10 animate-fade-in space-y-2">
      ...
      {/* Notes panel */}
      {showNotes && (
        <div className="px-4 py-3 border-b border-gray-100 dark:border-gray-800 bg-[#D4AF37]/5 dark:bg-[#D4AF37]/10 animate-fade-in">
            ...
            <div key={item.pos_product_id} className="flex items-center gap-3 p-3 bg-gray-50 dark:bg-gray-800/50 rounded-xl border border-gray-100 dark:border-gray-800 group">
              <div className="flex-1 min-w-0">
                <div className="flex justify-between items-start">
                  ...
                  <span className="font-bold text-sm text-[#2D6A4F] dark:text-[#52B788] ml-2 shrink-0">
              ...
              <div className="flex bg-white dark:bg-gray-700 rounded-lg shadow-sm border border-gray-200 dark:border-gray-600 overflow-hidden shrink-0">
                ...
                <button onClick={() => onUpdateQuantity(item.pos_product_id, 1)} className="p-1.5 hover:bg-[#2D6A4F]/10 hover:text-[#2D6A4F] dark:hover:bg-[#2D6A4F]/20 transition-colors">
                  <Plus size={14} />
                </button>
              </div>
      ...
      <div className="p-4 bg-gray-50 dark:bg-gray-800/80 border-t border-gray-200 dark:border-gray-800 backdrop-blur-md space-y-1.5">
        ...
        {discountAmount > 0 && (
          <div className="flex justify-between text-sm text-[#2D6A4F] dark:text-[#52B788]">
        ...
        <div className="flex justify-between text-xl font-black text-gray-900 dark:text-gray-50 pt-2 border-t border-gray-200 dark:border-gray-700">
          <span>Total</span>
          <span className="text-[#2D6A4F] dark:text-[#52B788]">KES {total.toLocaleString(undefined, { minimumFractionDigits: 2 })}</span>
        </div>

        <button
          onClick={onCheckout}
          disabled={items.length === 0}
          className="w-full py-3.5 mt-2 bg-gradient-to-r from-[#1B4332] to-[#2D6A4F] hover:from-[#0D1F16] hover:to-[#1B4332] border border-[#D4AF37]/30 disabled:opacity-50 disabled:cursor-not-allowed text-[#D4AF37] font-bold rounded-xl shadow-lg hover:shadow-xl transition-all transform active:scale-[0.98] flex justify-center items-center gap-2 text-base"
        >
